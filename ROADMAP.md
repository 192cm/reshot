...
- Google Drive 업로드 설정이 켜져 있으면 최종 이미지를 Drive 폴더에 업로드하고 QR 대상 URL을 Drive 공유 링크로 바꾼다.
...
시작 화면
  ...
  -> Save final 클릭
  -> final.jpg 합성 + 선택적 Drive 업로드 + qr.png 생성
  -> 결과 화면 이동
  -> QR 보기 또는 처음으로 이동
```
...
- QR은 기본적으로 `/sessions/{session_id}/image`를 가리키며, Google Drive 업로드가 켜져 있으면 Drive 공유 링크를 가리킨다.
...
| --- | --- | --- | --- |
| `GET` | `/` | 구현 | 기본 상태 확인 |
| `GET` | `/health` | 구현 | 서버 상태, 카메라 모드, 저장 경로, Google Drive 설정 상태 확인 |
...
| `POST` | `/sessions/{session_id}/compose` | 구현 | 선택한 4장 합성, 선택적 Drive 업로드, QR 생성 |
...
`session.json`에는 `qr_target_url`, `drive_file_id`, `drive_share_url`이 포함될 수 있다. Google Drive 업로드를 쓰지 않으면 Drive 관련 값은 `null`이고, QR 대상은 로컬 백엔드 이미지 URL이다.
...
- [x] 최종 이미지 저장
- [x] QR 이미지 생성 및 조회
- [x] Google Drive 선택 업로드와 공유 링크 기반 QR 대상 URL 저장
- [x] 세션 메타데이터 저장

### 운영 보조

- [x] `.env.example` 기본값 정리
- [x] Google Drive 환경변수 예시 추가
- [x] `frontend/.env.example` 추가
- [x] NX Tether 창 포커스 및 `Ctrl+1` 전송 스크립트
- [x] Google Drive OAuth 토큰 생성 스크립트
- [x] 로컬 네트워크 QR 설정 문서화
...
- [ ] Google Drive OAuth, 업로드 폴더 권한, 공개 공유 링크 검증
...
- OAuth 토큰이 없거나 scope가 바뀜
- `GOOGLE_DRIVE_FOLDER_ID`가 비어 있거나 권한이 없음
- 행사장 네트워크에서 Google API 접근이 불안정함
- 공개 공유 권한 생성이 조직 정책으로 차단됨

대응:

- 행사 전 `python -m app.scripts.google_drive_auth`로 토큰 생성 및 갱신 확인
- `/health`에서 `google_drive.enabled`, `folder_configured`, `share_public` 확인
- Drive 업로드 실패 시 로컬 QR 모드로 전환할 수 있도록 `GOOGLE_DRIVE_ENABLED=false` fallback 유지
- 공유 정책이 제한된 계정이면 업로드 폴더 권한과 링크 접근 정책을 사전 점검
...
- [ ] `/health` 응답과 `google_drive` 설정 상태 확인
- [ ] 세션 생성 API 확인
...
- [ ] final.jpg 생성 확인
- [ ] qr.png 생성 확인
- [ ] `GOOGLE_DRIVE_ENABLED=false`에서 QR 대상이 로컬 이미지 URL인지 확인
- [ ] `GOOGLE_DRIVE_ENABLED=true`에서 Drive 업로드, `drive_share_url`, `qr_target_url` 저장 확인
...
- QR을 휴대폰으로 스캔해 로컬 이미지 URL 또는 Drive 공유 링크의 결과 이미지를 열 수 있다.
...
5. Google Drive 업로드/공유 링크 모드 검증
6. 실패 복구 UI와 운영자 가이드 보강
